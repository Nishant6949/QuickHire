"""Email verification used once when a new employer or applicant signs up."""
...
def pending_account():
    """Get the new account waiting for sign-up email verification."""
...
def clear_pending_login():
    """Remove temporary sign-up verification values after verification is complete."""
    keys = (
        "pending_2fa_type",
        "pending_2fa_id",
        "pending_2fa_channel",
...
def verify():
    """Check the six-digit sign-up OTP and finish account creation."""
    account_type, account = pending_account()
    if account is None:
        clear_pending_login()
        return redirect(url_for("landing.index"))
    ...
    if request.method == "POST":
        ...
        # After successful sign-up verification, sign the new account in.
        if account_type == "employer":
