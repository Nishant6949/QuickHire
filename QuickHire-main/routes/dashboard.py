...
@login_required
def dashboard():
    jobs = db.session.execute(db.select(Job).where(Job.user_id == current_user.id).order_by(Job.created_at.desc())).scalars().all()
    job_ids = [j.id for j in jobs]
    applicants = db.session.execute(db.select(Candidate).where(Candidate.job_id.in_(job_ids)).order_by(Candidate.created_at.desc())).scalars().all() if job_ids else []
    now = __import__('datetime').datetime.now(__import__('datetime').timezone.utc).replace(tzinfo=None)
    active_jobs = [j for j in jobs if j.status in ('open','ready','processing','completed') and (not j.application_deadline or j.application_deadline >= now)]
    shortlisted = [c for c in applicants if c.status in ('shortlisted','invited','interview_done')]
    interviews = [c for c in applicants if c.status in ('invited','interview_done')]
    hired = [c for c in applicants if c.status == 'final_hired']
    scored = [c.match_score for c in applicants if c.match_score is not None]
    avg_score = round(sum(scored)/len(scored)) if scored else 0
    upcoming_deadlines = sorted([j for j in active_jobs if j.application_deadline], key=lambda j:j.application_deadline)[:5]
    recent_applicants = applicants[:6]
    return render_template('dashboard/dashboard.html', active_page='dashboard', active_jobs=len(active_jobs), total_applicants=len(applicants), shortlisted=len(shortlisted), interviews=len(interviews), hired=len(hired), avg_score=avg_score, upcoming_deadlines=upcoming_deadlines, recent_applicants=recent_applicants)
